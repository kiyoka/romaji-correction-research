...
GPT-5.2に加えて、Apple Intelligence（3Bモデル、オンデバイスLLM）でも同様のベンチマークを実施し、性能比較を行いました。詳細な比較グラフは `RESULT.md` に掲載していますが、Apple Intelligenceの正解率は28.57%にとどまり、現時点では実用レベルには達していません。GPT-5.2のようなフロンティアモデルとの性能差が大きいことが確認されました。
