...
## macOS 27.0での追加計測（2026-09-19）

macOS 26.2での測定を、macOS 27.0・Xcode 27.0のMacBook Air（M4）で再実行した。Swift実装、テストデータ、`PROPER_NOUN_AWARE_V6_ENGLISH` プロンプトは変更していない。各ケースで新しい `LanguageModelSession` を作り、正解候補との完全一致で評価した。全46件を同じ設定で続けて2回測定した。

| 測定 | 評価件数 | 完全一致 | 平均編集距離 | 平均応答時間 |
|---|---:|---:|---:|---:|
| macOS 26.2（2026-01-12） | 45/46件 | 21/45（46.67%） | 1.98 | 0.70秒 |
| macOS 27.0・1回目 | 46/46件 | 31/46（67.39%） | 1.57 | 1.13秒 |
| macOS 27.0・2回目 | 46/46件 | 31/46（67.39%） | 1.41 | 0.74秒 |

macOS 26.2では `srigtou` がエラーで結果に含まれなかった。**両世代で評価できた同じ45件**に絞ると、21/45（46.67%）から31/45（68.89%）へ10件、22.22ポイント増加した。macOS 27.0では `srigtou` も処理できたが、不正解だった。2回の正解ケース集合は同じで、出力の文面と時間には一部違いがあった。

| データセット | macOS 26.2 | macOS 27.0（共通45件） | macOS 27.0（全46件） |
|---|---:|---:|---:|
| 実データ | 1/8（12.5%） | 2/8（25.0%） | 2/8（25.0%） |
| 仮想データ | 5/9（55.6%） | 8/9（88.9%） | 8/10（80.0%） |
| 固有名詞 | 15/28（53.6%） | 21/28（75.0%） | 21/28（75.0%） |
| 合計 | 21/45（46.67%） | 31/45（68.89%） | 31/46（67.39%） |

以前は不正解で今回は正解したケースが12件（`shinkanswn → 新幹線`、`github → GitHub` など）、逆に不正解になったケースが2件（`android → アンドロイド`、`javascript → javascript`）だった。実データは依然として2/8件しか正解せず、`kanserushimazu → がんしまず`、`roukaidesu → よろしくお願いします` のように入力意図から離れる例が残る。

1回目と2回目で平均応答時間が異なるため、macOS 26.2の0.70秒との比較だけで速度の向上・低下は判断しない。また、OS以外の条件を独立に固定した実験ではなく、測定値の上昇をOS更新だけの効果とは断定できない。GPT-5.2の過去の41/42（97.62%）は、プロンプトがV2、ケース集合が42件と異なる参考値であり、同一条件でのモデル比較ではない。この測定もMac上の独立したプログラムによるもので、iOS版Sumibi本体やiPhone実機の性能を示すものではない。

**生データ:** [1回目CSV](results/experiment_results_apple_20260919_135113.csv) / [1回目サマリー](results/summary_apple_20260919_135113.txt) / [2回目CSV](results/experiment_results_apple_20260919_135209.csv) / [2回目サマリー](results/summary_apple_20260919_135209.txt)。比較元は [macOS 26.2のCSV](results/experiment_results_apple_20260112_221331.csv) と [サマリー](results/summary_apple_20260112_221331.txt)。

---

## 過去の実験概要（GPT-5.2、2025年12月）
...
## Apple Intelligence（Foundation Models）の過去の測定結果（macOS 26.2）

以下の評価と結論は2026年1月時点の記録。macOS 27.0での再測定結果は、この文書の冒頭に記載した。
